...
test("Pipeline Stack Test", () => {
  ...
  expect(
    pipelines[pipelineLogicalId].Properties.ArtifactStore.Location.Ref
  ).toMatch(/^dataportalstatuspageartifactbucket/);
  ...
  const codeBuildProjectLogicalId = Object.keys(codeBuildProjects).find(
    (id) =>
      codeBuildProjects[id].Properties.Name === "DataPortalStatusPageReactBuild"
  );
        ...
        Match.objectLike({
          TargetAddress: Match.objectLike({
            Ref: Match.stringLikeRegexp(
              "^SsmParameterValuedataportalbackendnotificationsnstopicarn"
            ),
          }),
          TargetType: "SNS",
        }),
